...
```{r}
rst_Random  <- rast("../data_share/rst_Extract_Random.asc")

vct_Grid    <- vect("../data_share/vct_Extract_Grid.geojson")
vct_Region    <- vect("../data_share/vct_Extract_Region.geojson")
vct_Point   <- vect("../data_share/vct_Extract_Point.geojson")
vct_Voronoi <- vect("../data_share/vct_Extract_Voronoi.geojson")
...
# Calculate the mean value of the attribute 'lyr.1' in the first intersected polygon
# 'lyr.1' comes from the original raster values when it was converted to polygons
mean(vct_Random_Poly1$lyr.1)

# Calculate the mean value of the attribute 'lyr.1' in the second intersected polygon
mean(vct_Random_Poly2$lyr.1)
...
gp_CropExact_Poly1 <- ggplot() +
  geom_spatvector(data = vct_Random_Poly1, aes(fill = lyr.1)) +
...
gp_CropExact_Poly2 <- ggplot() +
  geom_spatvector(data = vct_Random_Poly2, aes(fill = lyr.1)) +
  geom_spatvector(data = vct_Region[2], color = color_TUD_pink, linewidth = .5, fill = NA) +
  scale_fill_gradientn("Value", 
                       colors = color_DRESDEN, 
                       na.value = "transparent",
